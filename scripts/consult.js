...
const DEFAULT_TARGET = 'egc';
const DEFAULT_LIMIT = 5;
const MAX_LIMIT = 20;
const SCHEMA_VERSION = 'egc.consult.v1';
...
function showHelp(exitCode = 0) {
  console.log(`
Consult EGC install components and profiles from any project
...
function commandFor(kind, id, target) {
  if (kind === 'profile') {
    return `npx egc install --profile ${id} --target ${target}`;
  }

  return `npx egc install --profile minimal --target ${target} --with ${id}`;
}

function planCommandFor(componentId, target) {
  return `npx egc plan --profile minimal --target ${target} --with ${componentId}`;
}
...
function buildConsultation(options) {
  ...
  return {
    ...
    nextSteps: matches.length > 0
      ...
      : [
        'Run `npx egc catalog components` to browse all components.',
...
function formatText(payload) {
  const lines = [
    `EGC consult (${payload.generatedAt})`,
    `Query: ${payload.query}`,
