"""Run scenarios via egc -p and parse tool calls from stream-json output."""
...
def run_scenario(
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"egc -p failed (rc={result.returncode}): {result.stderr[:500]}"
        )
...
def _parse_stream_json(stdout: str) -> list[ObservationEvent]:
    """Parse egc -p stream-json output into ObservationEvents.
